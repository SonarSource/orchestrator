...
public class PackagingResolverTest {
  ...
  public void fail_if_version_cant_be_guessed_from_local_zip() throws Exception {
    File zip = temp.newFile("sonar.zip");
    when(locators.locate(any())).thenReturn(zip);
    SonarDistribution distribution = new SonarDistribution().setZipLocation(FileLocation.of(zip));
  ...
  private void testResolutionOfEdition(Edition edition, String editionGroupId, String editionArtifactId, String versionOrAlias, String returnedVersion) throws IOException {
    ...
    assertThat(packaging.getVersion().toString()).isEqualTo(returnedVersion);
  ...
  private File prepareResolutionOfZip(String groupId, String artifactId, String version) throws IOException {
    ...
    File zip = new File(dir, "sonarqube-" + version + ".zip");
    FileUtils.touch(zip);
