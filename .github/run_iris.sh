...
function run_iris_next () {
  java \
    -Diris.source.projectKey="org.sonarsource.orchestrator:orchestrator-parent" \
    -Diris.source.url="$SONAR_NEXT_URL" \
    -Diris.source.token="$SONAR_IRIS_NEXT_TOKEN" \
    -Diris.destination.projectKey="SonarSource_orchestrator" \
...
function run_iris_sqc_us () {
  java \
    -Diris.source.projectKey="SonarSource_orchestrator" \
